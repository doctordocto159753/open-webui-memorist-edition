...
export class MemoristImportWorkflow {
  ...
  private renderProcessingReport(report: ImportProcessingReport): void { this.processingFailureCount = report.processing_jobs_failed; const el = this.root.querySelector<HTMLElement>("[data-processing-report]")!; el.hidden = false; const providers = report.provider_breakdown.map((item) => `${item.operational_role}${item.prompt_role ? ` / ${item.prompt_role}` : ""}: ${item.provider_type}/${item.model_name} (${item.processing_jobs} jobs, ${item.input_tokens}/${item.output_tokens} tokens)`).join("; "); el.innerHTML = `<h3>Processing report</h3><dl>${row("Imported conversations", report.imported_conversations)}${row("Imported messages", report.imported_messages)}${row("Total processing jobs", report.processing_jobs_total)}${row("Queued", report.processing_jobs_queued + report.processing_jobs_pending)}${row("Retry scheduled", report.processing_jobs_retry_scheduled)}${row("Running", report.processing_jobs_running)}${row("Succeeded", report.processing_jobs_succeeded)}${row("Failed", report.processing_jobs_failed)}${row("Skipped", report.processing_jobs_skipped)}${row("Already processed", report.processing_jobs_already_processed)}${row("Memory candidates", report.memory_candidates_created)}${row("Memory versions", report.memory_versions_created)}${row("Prompt executions", report.prompt_execution_runs)}${row("Usage events", report.model_usage_events)}${row("Input tokens", report.input_tokens)}${row("Output tokens", report.output_tokens)}${row("Graph projection events", report.graph_projection_outbox_events)}${row("Providers", providers)}${row("Skip reasons", formatReasons(report.skip_reasons))}${row("Sanitized last error", sanitizeUiError(report.last_error_sanitized || "Not reported"))}${row("Final status", report.final_status)}</dl>`; this.button("retry").disabled = this.processingFailureCount < 1; }
  ...
  private stopLoops(): void { if (this.pollTimer) clearTimeout(this.pollTimer); this.pollTimer = null; this.abortController?.abort(); this.abortController = null; }
...
function sanitizeUiError(error: unknown): string { const text = error instanceof Error ? error.message : String(error || "Import failed."); return text.replace(/Bearer\s+[^\s,;&}\])]+/gi, "Bearer [redacted]").replace(/((?:x-)?api[_-]?key|token|password|secret(?:_env_var_name)?|authorization)(\s*[:=]\s*)[^\s,;&}\])]+/gi, "$1$2[redacted]").replace(/https?:\/\/[^\s<>"']+/gi, "[redacted-url]").replace(/sk-[A-Za-z0-9_-]+/g, "[redacted]").replace(/\/[^\s]+/g, "[path]"); }
